...
cv::Mat preprocessing::get_sonar_mask( const std::vector<float>& bearings, uint32_t bin_count, uint32_t beam_count, uint32_t frame_width, uint32_t frame_height) {
    ...
    for (size_t i = 0; i < beam_mapping.size(); i++){
        int y = i / frame_width;
        int x = i % frame_width;
        if (beam_mapping[i] > -1)  mask.at<uchar>(y, x) = 255;
    }
    return mask;
}

void preprocessing::extract_roi_masks (const cv::Mat& sonar_image,
                        const std::vector<float>& bearings,
                        uint32_t bin_count, uint32_t beam_count,
                        cv::Mat& roi_cart, cv::Mat& roi_polar,
                        float alpha) {

    cv::Mat mask = preprocessing::get_sonar_mask(bearings, bin_count, beam_count, sonar_image.cols, sonar_image.rows);
    ...
    for (size_t i = 0; i < sonar_image.rows; i++) {
        ...
        row_mean[sonar_image.rows - 1 - i] = value;
    }
    ...
    for (size_t i = 0; i < roi_polar.rows; i++) {
        for (size_t radius = 0; radius < roi_polar.cols; radius++) {
            ...
            if (y < new_y) roi_polar.at<uchar>(i, radius) = 0;
            else break;
        }
    }
}
