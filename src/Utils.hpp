...
namespace sonar_processing {
    ...
    namespace utils {
        ...
        inline cv::Rect
        clip_rect(const cv::Point &tl, const cv::Point &br, const cv::Point &min_tl, const cv::Point &max_br) {
        ...
        inline cv::Rect neighborhood_rect(int x, int y, int n, int w, int h) {
            return clip_rect(cv::Point(x - n, y - n), cv::Point(x + n, y + n), cv::Point(0, 0),
                             cv::Point(w - 1, h - 1));
        }
        ...
        inline static std::vector<float> get_radians(const std::vector<base::Angle> &angles) {
            ...
            for (std::vector<base::Angle>::const_iterator it = angles.begin(); it != angles.end(); it++)
                radians.push_back(it->rad);
            return radians;
        }
